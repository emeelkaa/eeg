...
model = TSception().to(device)
criterion = nn.BCEWithLogitsLoss()
optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=15, factor=0.5, verbose=True)
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        early_stop_counter = 0
        torch.save(model.state_dict(), "best_tsception.pth")
...
model.load_state_dict(torch.load("best_tsception.pth"))
...
logging.info("Confusion matrix saved as 'cm_tsception.png'")
